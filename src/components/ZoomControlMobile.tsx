...
export default function ZoomControlMobile() {
  ...
  return (
    <div className="flex items-center space-x-1 bg-white border border-gray-200 rounded-lg px-2 py-1 shadow-sm">
      <Button
        variant="ghost"
        size="sm"
        onClick={() => applyZoom(zoom - 10)}
        className="h-8 w-8 p-0 hover:text-blue-600 hover:bg-blue-50 transition-colors touch-target no-zoom"
        title="Diminuir Zoom"
        disabled={zoom <= 50}
      >
      ...
      <span className="w-8 text-center text-xs font-medium text-gray-700 select-none">
        {zoom}%
      </span>
